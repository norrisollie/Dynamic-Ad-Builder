...
# Compute absolute project dir and print actions
  ...
  # Try package-provided clean first (if present), then run example build
  (cd "$dir" && (npm run prebuild:example || true) && npm run build:example) || continue
  ...
  ZIP_PATH="$RUN_DIR/${SIZE}.example.${TIMESTAMP}.zip"
