# 320x50 Template & Feed Notes

This file documents the template keys that 320x50 can use and how the feed rows should be structured, using the existing ad feed as the reference.
...
The 320x50 size follows the same feed patterns as the other sizes in the reference sheet (300x250, 160x600, 300x600, 320x250, 728x90, 970x250). Only the asset paths change to use `320x50_…`.

### Shared columns

- `creative_dimensions` – e.g. `320x50`.
...
- Frame usage
    ...
    - `f2_image.Url`, `f3_image.Url`, `f4_image.Url` – size-specific image paths (e.g. `2025/attract/320x50_ratings_attract_travel_image_f2.jpeg`).
    - `f2_cta`, `f3_cta`, `f4_cta` – always `Apply now` in the sheet.

    #### Sticky reference: Attract Engineers – Professional Benefits / Development
    - `reporting_label` – `320x50_Royal_Navy_Attract_Engineers_Professional_Benefits_Development`.
    ...
    - Additional override – the feed also sets `.frame.template-attract#frame-1 .frame__searchbar .frame__copy-icon-wrapper {\n    opacity: 0;\n    flex: 0 0 20px;\n    align-items: center;\n    width: 100%;\n    height: 100%;\n    display: flex;\n}` to keep the 320x50 search icon container hidden and stretched.
    - Frame 1 copy – `Jobs that pay me to skill up` with `f1_image.Url = empty.png` and no CTA.
    - Frame 2 copy – `FUTURE SHAPER` + CTA `Apply now` + image `2025/attract/320x50_engineers_attract_development_image_f2.jpeg`.
    - Frame 3 copy – `EDGE GAINER` + CTA `Apply now` + image `2025/attract/320x50_engineers_attract_development_image_f3.jpeg`.
    - Frame 4 copy – `BECOME A SPECIALIST` + CTA `Apply now` + image `2025/attract/320x50_engineers_attract_development_image_f4.jpeg`.
    - CTA copy is consistent with the rest of the attract rows; no `global_cta` is defined here.
    - Local preview – [src/scripts/environments/applyDevDefaults.js](src/scripts/environments/applyDevDefaults.js#L1-L200) seeds this row, so keep it synchronized with the feed entry above.
...
- Frame usage
    ...
    - `f1_image.Url` – the hero image for the size, e.g. `2025/convert/320x50_ratings_convert_pay_image.jpeg`.
    - `f1_cta` – `Apply now`.
