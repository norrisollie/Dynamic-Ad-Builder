...
for dir in */; do
  ...
  SIZE="${dir%/}"
  echo "---- Building $SIZE (development) ----"
...
# Compute absolute project dir and print actions
  ...
  # Try package-provided clean first (if present), then run build
  (cd "$dir" && (npm run prebuild:development || true) && npm run build:development) || continue
  ...
  ZIP_PATH="$RUN_DIR/${SIZE}.development.${TIMESTAMP}.zip"
