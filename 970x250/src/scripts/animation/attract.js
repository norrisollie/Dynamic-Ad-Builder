...
export const attractAnimation = (formattedFeed) => {
    ...
    if (includesFrame1) {
        ...
        // split text for f1 animation (lines and chars, with per-line cursors)
        splitTextF1 = SplitText.create(".frame-1__copy1-text", {
            ...
            onSplit: () => {
                ...
                // hiding init state - SplitText sets inline-block, so hide via GSAP
                gsap.set(".frame-1__copy1-text .frame__split-chars", {
                    display: "none",
                    scale: 0,
                });

                // create a cursor element to clone per line
    ...
    const playAnimation = () => {
        ...
        frameOrderArray.forEach((frameNumber, index) => {
            ...
            if (frameNumber === "1") {
                ...
                // Animate each line's characters sequentially with per-line cursor
                ...
                copyLines.forEach((line, lineIndex) => {
                    ...
                    tl.to(
                        lineChars,
                        {
                            ...
                            duration: 0,
                            onStart: () => {
                                // show cursor on this line and start blinking
                                if (lineCursor) {
                            ...
                            onComplete: () => {
                                // hide cursor when line done, except last line
                                if (!isLastLine && lineCursor) {
